...
-- ##   🛑🛑🛑  禁止在有数据的库上执行本文件  🛑🛑🛑                    ##
-- ##                                                                 ##
-- ##   本文件是「空库初始化脚本」，满篇 DROP TABLE + CREATE TABLE。    ##
-- ##   在已有数据的库（尤其 prod）上跑一遍 = 业务数据全部清零。        ##
-- ##                                                                 ##
-- ##   🔴 真实事故：2026-07-31 10:39，prod 上 67 张表被本文件重建，    ##
-- ##      题库域全清（15662 道题 / 3400 KG 节点 / 全部答案解析），     ##
-- ##      书架 13101 条题目引用全部悬空。当日 21:00 从 dev 恢复。      ##
-- ##      起因＝执行部署 DDL 时抓错了文件。                            ##
-- ##                                                                 ##
-- ##   ✅ 该跑什么：部署新增字段/新表 → 跑那一批的                     ##
-- ##      `下次部署需要的文件/<PRD-id>/prod-DDL-*.sql`                ##
-- ##      （那种文件只有 ALTER / CREATE TABLE IF NOT EXISTS，幂等安全）##
-- ##                                                                 ##
-- ##   ✅ 本文件的正当用途：① 查 schema 事实（读，不执行）             ##
-- ##                       ② 全新空库初始化（确认库里没数据）          ##
-- ##                                                                 ##
-- ##   执行前自查一句：                                                ##
-- ##     SELECT COUNT(*) FROM biz_question;  -- 非 0 就绝对不要跑      ##
-- ##                                                                 ##
-- #####################################################################
-- =====================================================================
